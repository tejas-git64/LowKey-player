...
export default function UserPlaylist() {
  ...
  function getUserPlaylist() {
    playlistData = userContext.yourPlaylists[0].find(song => song.playlistName === playlistName)
  ...
  return (
    <>
      <div className="w-full lg:w-[calc(100%-5%)] 2xl:w-[calc(100%+44%)] mx-auto h-full top-0 left-0 bg-[conic-gradient(at_top,_var(--tw-gradient-stops))] from-neutral-800 to-black">
        <div className="w-full h-auto relative border-b-2 border-gray-700 flex justify-between items-end py-8 px-4 pr-6 bg-gradient-to-t from-black to-teal-500">
        <Link to="/home" className="w-auto h-auto right-5 top-3 absolute">
          ...
          <span className="w-auto h-auto -mt-3 lg:w-96 flex flex-col items-start lg:flex-row lg:items-center">
            <img src={userPlaylist.songs[0].image[2].link} alt="playlist-image" className="w-[110px] h-[110px] lg:w-32 lg:h-32 ml-2 mr-5"/>
            <h2 className="w-52 h-auto text-white text-xl lg:text-3xl my-2 pl-2 lg:pl-0 font-bold text-left whitespace-pre-line lg:whitespace-nowrap">{userPlaylist.playlistName}</h2>
          </span>
          <button
        ...
        <ul className="w-full h-[calc(100%-23%)] py-2 md:p-4 overflow-y-scroll scroll-smooth">
          {userPlaylist.songs.map((usong)=><Song usong={usong} setPsPlay={setPsPlay} key={usong.id} playlistSong={userPlaylist}/>)}
        </ul>
      </div>
    </>
  );
